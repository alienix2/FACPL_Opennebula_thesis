...
public class PolicySet_SLA_Type2 extends PolicySet {
	public PolicySet_SLA_Type2(){
		...
		)),new ExpressionBooleanTree(new ExpressionFunction(new it.unifi.facpl.lib.function.comparison.Equal(), "3",new AttributeName("resource","vm-type") 
		)))
		);
		//PolElements
		...
		private class Rule_hyper_2_create extends Rule {
				...
				Rule_hyper_2_create (){
					...
					//Obligations
					addObligation(new Obligation("create",Effect.PERMIT,ObligationType.M,5,
					new AttributeName("system","vm-id") ,
					"3"
					)
					);
